...
describe("acquireTelegramVerticalSwipeLock", () => {
  ...
  it("restores the host gesture only after every owner releases its lock", () => {
    const telegram = installTelegram("android");

    const releaseFirst = acquireTelegramVerticalSwipeLock();
    const releaseSecond = acquireTelegramVerticalSwipeLock();
    releaseFirst();

    expect(telegram.disableVerticalSwipes).toHaveBeenCalledOnce();
    expect(telegram.enableVerticalSwipes).not.toHaveBeenCalled();
