...
- Add Word and Edit share a meaning editor without a plus button. Typing offers one more empty field until eight meanings are filled. Any populated meaning can be deleted or reordered with its handle or Move up / Move down controls. While dragging, neighboring meanings shift to show the exact resulting order before release. The empty field is optional, does not participate in reordering, and is never saved.
