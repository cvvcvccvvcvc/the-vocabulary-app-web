...
- On a touch phone, focusing an Add Word field hides the bottom navigation. The card is
  centered when idle, stays clear of Telegram's controls, and scrolls the selected field
  into view when space shrinks. Add Word stays close above the software keyboard. A narrow
  desktop window keeps its navigation while typing.
...
- Words can be searched and sorted by date added, alphabetically, or by learning level from a compact custom menu. Date defaults to newest first, text to A–Z, and level to 0–9; choosing the active criterion again reverses it. Swiping a row left progressively reveals a rounded Delete action without opening the word. A medium swipe leaves it available to tap and attached to its row during list scrolling; a deep swipe expands the action and opens confirmation directly. Mobile Telegram's vertical close gesture is disabled while Words is open. Deletion always requires confirmation.
