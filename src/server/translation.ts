...
function uniqueMeanings(values: readonly string[], maximum: number): string[] {
...
function googleMeanings(body: unknown, maximum: number): string[] {
  ...
  return uniqueMeanings([translated, ...alternatives], maximum);
}

function yandexMeanings(body: unknown, pair: string, maximum: number): string[] {
  ...
  return uniqueMeanings(translations, maximum);
}
...
export class TranslationService {
  ...
  async suggest(text: string, settings: LanguageSettings): Promise<string[]> {
    ...
    const maximum = settings.translationMaxMeanings;
    if (settings.translationMethod === "yandex") {
      ...
      return this.suggestFrom(url, (body) => yandexMeanings(body, pair, maximum), "Yandex");
    }
    ...
    return this.suggestFrom(url, (body) => googleMeanings(body, maximum), "Google");
  }
